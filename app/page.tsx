...
export default function Home() {
  ...
  useEffect(() => {
    ...
    async function hydrateFromServer() {
      ...
      const billingResponse = await fetch("/api/billing", { credentials: "include" }).catch(() => null);
      const billingData = billingResponse?.ok
        ? ((await billingResponse.json()) as Partial<BillingState>)
        : null;
      if (cancelled) return;
      ...
      setWorkspace((current) => ({
        ...
        billing: billingData
          ? {
              plan: billingData.plan ?? current.billing.plan,
              status: billingData.status ?? current.billing.status,
              currentPeriodEnd: billingData.currentPeriodEnd ?? current.billing.currentPeriodEnd,
              cancelAtPeriodEnd: billingData.cancelAtPeriodEnd ?? current.billing.cancelAtPeriodEnd,
            }
          : current.billing,
      }));
...
function PricingView({
  ...
  overageEnabled: boolean;
  startCheckout: (plan: Extract<PlanKey, "launch" | "team" | "business">) => void;
}) {
  return (
    <section className="pricing-grid">
      {(Object.keys(planCatalog) as PlanKey[]).map((plan) => {
        ...
        return (
          <article className={activePlan === plan ? "panel pricing-panel active" : "panel pricing-panel"} key={plan}>
            ...
            {plan === "enterprise" ? (
              <button className="button secondary wide" disabled type="button">Contact sales</button>
            ) : plan === "free" ? (
              <button className="button secondary wide" onClick={() => changePlan(plan)} type="button">Use Free</button>
            ) : billing.plan === plan && billing.status !== "inactive" ? (
              <button className="button secondary wide" onClick={openBillingPortal} type="button">Manage billing</button>
            ) : (
              <button className="button primary wide" onClick={() => startCheckout(plan)} type="button">Start secure checkout</button>
            )}
          </article>
        );
      })}
      <div className="panel pricing-note">
        <h3>Usage model</h3>
        <p>Billing status: {billing.status}. {billing.cancelAtPeriodEnd ? "Cancellation is scheduled at period end." : "Subscriptions renew through Stripe."}</p>
        <p>
